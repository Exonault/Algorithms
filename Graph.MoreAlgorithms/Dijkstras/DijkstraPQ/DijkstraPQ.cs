...
using Graph.MoreAlgorithms.Dijkstras.Dijkstra1;

namespace Graph.MoreAlgorithms.Dijkstras.DijkstraPQ
{
    public class DijkstraPQ
    {
        public void DijkstraPQMain()
        {
            ...
            var graph = new Dictionary<Node, Dictionary<Node, int>>()
            {
                ...
                {
                    nodes[4],
                    new Dictionary<Node, int>() {{nodes[1], 20}, {nodes[5], 5}, {nodes[6], 17}, {nodes[11], 11}}
                },
                ...
                {
                    nodes[7],
                    new Dictionary<Node, int>() {{nodes[1], 26}, {nodes[2], 15}, {nodes[9], 3}, {nodes[11], 20}}
                },
            ...
            int sourceNode, int destinationNode)
        {
            ...
            var path = DijkstraPQAlgorithm(graph, nodes[sourceNode], nodes[destinationNode]);
        ...
        private static List<int> DijkstraPQAlgorithm(Dictionary<Node, Dictionary<Node, int>> graph, Node sourceNode,
            Node destinationNode)
        {
            ...
            {
                var currNode = priorityQueue.ExactMin();
                ...
                foreach (var edge in graph[currNode])
                {
                    ...
                    double distance = currNode.DistanceFromStart + edge.Value;

                    if (distance < edge.Key.DistanceFromStart)
                    {
                        edge.Key.DistanceFromStart = distance;
                        previous[edge.Key.Id] = currNode.Id;
